...
def _section_pcbdraw(pcbdraw_front: Optional[Path], pcbdraw_back: Optional[Path],
                     styles: Dict) -> List:
    """Optional stylized 2D view (pcbdraw) — copper + silk + outline.

    Complements the 3D photoreal renders in section 3.0; this is the
    schematic-style flat view useful for visualising routing.
    """
    if not (pcbdraw_front and pcbdraw_front.exists()) and \
       not (pcbdraw_back and pcbdraw_back.exists()):
        return []
    s = [SectionTitle("3.1", "PCB Stylized View"), Spacer(1, 4)]
    s.append(Paragraph(
        "Schematic-style 2D rendering generated by pcbdraw — copper "
        "tracks, silkscreen, board outline, and pad shapes in vector "
        "form. Useful for inspecting routing without the visual noise "
        "of the photoreal 3D view.",
        styles["body"]))
    s.append(Spacer(1, 8))

    cells = []
    if pcbdraw_front and pcbdraw_front.exists():
        cells.append(Image(str(pcbdraw_front), width=3.3 * inch, height=2.5 * inch,
                           kind='proportional'))
    if pcbdraw_back and pcbdraw_back.exists():
        cells.append(Image(str(pcbdraw_back), width=3.3 * inch, height=2.5 * inch,
                           kind='proportional'))
    if cells:
        t = Table([cells], colWidths=[3.5 * inch] * len(cells))
        t.setStyle(TableStyle([
        ...
        captions = []
        if pcbdraw_front and pcbdraw_front.exists():
            captions.append("Figure 3a. Front (stylized).")
        if pcbdraw_back and pcbdraw_back.exists():
            captions.append("Figure 3b. Back (stylized).")
        s.append(Paragraph("    ".join(captions), styles["caption"]))
    return s
